...
def create_spend_graph(df):
    fig = Figure(figsize=(15, 5))
    ax = fig.add_subplot(111)
    ax.bar(df['Category'], df['Withdrawal'])
...
def plotSpendGraph(df, canvas, figure):
    df = calcSpending(df)
    figure = create_spend_graph(df)
...
def create_income_graph(df):
    fig = Figure(figsize=(15, 5))
    ax = fig.add_subplot(111)
    ax.bar(df['Category'], df['Deposit'])
    ax.set_xlabel('Category')
...
def plotIncomeGraph(df, canvas, figure):
    df = calcIncome(df)
    figure = create_income_graph(df)
    canvas.get_tk_widget().pack_forget()
    canvas.get_tk_widget().pack(side=tk.BOTTOM, fill=tk.BOTH, expand=True)
    canvas.draw()
...
def createGUI(df):
    window = tk.Tk()
    window.title("Spending by Category")
    frame = tk.Frame(window)
    frame.pack(side=tk.TOP, fill=tk.X)
    btn1 = tk.Button(frame, text="Plot Spend", command=lambda: plotSpendGraph(df, canvas, figure))
    btn1.pack(side=tk.LEFT)
    btn2 = tk.Button(frame, text="Plot Income", command=lambda: plotIncomeGraph(df, canvas, figure))
    btn2.pack(side=tk.LEFT)
    figure = create_spend_graph(calcSpending(df))
